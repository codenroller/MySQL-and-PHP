...
--
ALTER TABLE `console`
ADD CONSTRAINT `uc_console` UNIQUE (`nom_console`); 
...
CREATE TABLE IF NOT EXISTS `jeux_video` (
  `id_jeux` SMALLINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
...
ADD CONSTRAINT `uc_jeux_video` UNIQUE (`nom_jeux`, `id_proprietaire`, `id_console`); 
...
(`id_jeux`, `nom_jeux`, `prix`, `nbre_joueurs_max`, `commentaires`, `id_proprietaire`, `id_console`, `date_enregistrement`) 
	VALUES
...
--
-- Adding unique constraint on proprietaire
--
ALTER TABLE `proprietaire`
ADD CONSTRAINT `uc_proprietaire` UNIQUE (`nom_proprietaire`); 
